...
| 服务 | 地址 | 用途 |
|---|---|---|
| HTTP REST | `http://192.168.120.30:8080` | 文件上传下载、任务管理、历史数据查询等（另行文档） |
| WebSocket | `ws://192.168.120.30:8080` | 实时遥测/状态/目标/告警推送、控制指令上行 |

**前端连接地址规则**（`wsClient.resolveWsUrl()`）：

- 优先读环境变量 `VITE_WS_URL`（完整地址，如 `ws://192.168.120.30:8080`）；
- 未配置时连**同源** `/ws` 路径——开发环境由 Vite dev server 代理转发（`vite.config.ts`）：

| 前端请求 | 代理转发至 |
|---|---|
| `ws://localhost:5173/ws` | `ws://192.168.120.30:8080` |
| `http://localhost:5173/api/xxx` | `http://192.168.120.30:8080/xxx` |
...
- [ ] 后端 WS 服务监听 `192.168.120.30:8080` 可达（telnet/curl 握手通过）
